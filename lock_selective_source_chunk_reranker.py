"""Lock the final BM25-default selective table gate before confirmatory sampling."""
...
def main() -> None:
    ...
    parser.add_argument("--development", default="outputs/selective_source_chunk_reranker_development_2026-07-15-v1/development_report.json")
    parser.add_argument("--test-log", required=True)
    parser.add_argument("--output", default="outputs/selective_source_chunk_reranker_method_lock_2026-07-15/method_lock_manifest.json")
    ...
    if development.get("dataset_role") != "combined_observed_development_only" or development.get("new_confirmatory_executed") is not False:
        raise ValueError("development report has an invalid role")
    ...
    files = [
        Path("source_chunk_reranker.py"), Path("develop_selective_source_chunk_reranker.py"),
        Path("evaluate_source_chunk_reranker.py"), Path("tests/test_source_chunk_reranker.py"),
        Path("docs/superpowers/specs/2026-07-15-source-chunk-reranking-design.md"),
        development_path, development_path.with_name("grid_results.json"), development_path.with_name("selected_per_query.json"),
        ...
        index, index.with_suffix(".meta.json"), index.with_name("variant_manifest.json"), test_log,
    ]
    ...
    manifest = {
        "schema_version": "1.0", "lock_status": "locked_before_confirmatory_set_construction",
        "locked_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "new_confirmatory_set_executed_before_lock": False,
        "method_family": "selective_table_gate",
        "development_role": "combined_90_already_observed_queries",
        "development_query_hashes": development["group_query_hashes"],
        "combined_development_query_sha256": development["combined_query_sha256"],
        "selected_parameters": development["selected"]["parameters"],
        "declared_methods": ["BM25_context_matched", "R1_raw", "Global_source_RRF", "Selective_table_gate"],
        "parameters": {
            "bm25_k1": 1.2, "bm25_b": 0.75,
            "source_payload": "parents_context+heading+content_first_2000_chars",
            "tokenization": "lowercase_ascii_alphanumeric", "embedding_model": "tencent/Youtu-Embedding",
            "dense_index": "IndexFlatIP_L2_normalized", "candidate_depth_per_channel": 30,
            "baseline_output_depth": 60, "rrf_k": 60, "bootstrap_iterations": 10000,
            "analysis_seed": 20260715, "confirmatory_sampling_seed": 20260716,
            "primary_outcome": "MRR_vs_BM25", "primary_minimum_effect": 0.03,
